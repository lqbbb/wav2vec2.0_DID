...
from ecapa_tdnn_2D import ECAPA_TDNN
...
class SSLModel(nn.Module):
    def __init__(self,device):
        ...
        cp_path = '/home/lqb/project/wav2vec2.0_DID/pre-trained_model_SSL/xlsr2_300m.pt'   # change the pre-trained XLSR model path as per your directoy
...
class Model(nn.Module):
    ...
    def __init__(self,device):
        ...
        filts =[128, [1, 32], [32, 32], [32, 64], [192, 192]]
        gat_dims =[64, 32]
        ...
        self.first_bn1 = nn.BatchNorm2d(num_features=192)
        ...
        #ZJ
        self.encoder = ECAPA_TDNN(in_channels=1,channels=64)  #torch.Size([4, 192, 42, 67])
        ...
        self.pos_S = nn.Parameter(torch.randn(1, 42, filts[-1][-1]))   # nn.Parameter 在参数优化的时候可以进行优化的
    ...
    def forward(self, x, Freq_aug=False):
        ...
        x=self.LL(x_ssl_feat) #(bs,frame_number,feat_out_dim)  ([2, 201, 128])

        #-------pre-trained Wav2vec model fine tunning ------------------------##

        x= x.transpose(1, 2)   #(bs,feat_out_dim,frame_number)
        x = x.unsqueeze(dim=1) # add channel ([2, 1, 128, 201])
        ...
        #------------SAP for spectral feature-------------# 光谱特征SAP
        w1=F.softmax(w,dim=-1)
        m = torch.sum(x * w1, dim=-1)
        
        e_S = m.transpose(1, 2) + self.pos_S 
        
        # graph module layer  图形模块层
        gat_S = self.GAT_layer_S(e_S)
        
        out_S = self.pool_S(gat_S)  # (#bs, #node, #dim)
        

        #------------SAP for temporal feature-------------# SAP for时间功能
